...
class Login {
    ...
    public function ExeLogin(array $UserData) {
        $this->CPF = (string) strip_tags(trim($UserData['user_cpf']));
	...
	public function getUserName()
		{
		$read->ExeRead("users", "WHERE user_cpf = :e", "e={$this->CPF}");
		
		return "Mayco Vilefort";
		}
    /*
     * ***************************************
     * **********  PRIVATE METHODS  **********
    ...
    private function setLogin() {
        if (!$this->CPF || !$this->Senha):
            $this->Error = ['Informe seu CPF e senha para efetuar o login!', WS_INFOR];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', WS_ALERT];
            $this->Result = false;
        elseif ($this->Result['user_level'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['user_nome']}, você não tem permissão para acessar esta área!", WS_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
...
//        $this->Senha = md5($this->Senha);
        ...
        $read = new Read;
        $read->ExeRead("users", "WHERE user_cpf = :e AND user_pass = :p", "e={$this->CPF}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['user_nome']}, seja bem vindo(a). Aguarde redirecionamento!", WS_ACCEPT];
        $this->Result = true;
    }
