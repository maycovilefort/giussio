...
<form name="Contas" class="j_FormCadastro" action="" method="POST" id="formContas">
    <div class="alert-box"></div>
    <input class="noclear" type="hidden" name="action" value="create"/>

    <div class="clearfix"></div>
    <h4 class="form-control label-info text-uppercase" style="color: #fff;">Quais os dados da Conta Bancárias?</h4>

    <div id="cont_banco" class="form-group col-md-6">
        <label>Banco:</label>
        <select name="cont_banco" class="form-control autocomplete">
            ...
            <option value="HSBC Bank Brasil S.A. - Banco Múltiplo(399)">HSBC Bank Brasil S.A. - Banco Múltiplo(399)
            </option>
    ...
    <div class="form-group col-md-6">
        <label>Titular:</label>
        <input type="text" name="cont_titular" id="cont_titular" class="form-control" placeholder="Digite o nome completo..."/>
    </div>

    <div class="form-group col-md-3">
        <label>CPF:</label>
        <input type="text" name="cont_cpf" class="form-control mask_cpf"/>
    </div>

    <div class="clearfix"></div>

    <div class="form-group col-md-12">
        <textarea name="cont_obs" class="form-control" placeholder="Suas observações aqui..." rows="7"></textarea>
    </div>
...
<script>
    ...
    $('.content').on('blur', '.j_cep', function () {
        ...
        $.getJSON(url, function (dadosRetorno) {
            try {
                $('input[name="cont_endereco"]').val(dadosRetorno.logradouro);
                $('input[name="cont_bairro"]').val(dadosRetorno.bairro);
                $('input[name="cont_cidade"]').val(dadosRetorno.localidade);
                $('input[name="cont_uf"]').val(dadosRetorno.uf);
                $('input[name="cont_num"]').focus();
            } catch (ex) {
            }
        });

        $('.content').on('blur', 'input[name="cont_comp"]', function () {
            $('input[name="cont_celular"]').focus();
        });
    });

    $('#cont_banco').change(function () {

        var codBanco = $('select[name="cont_banco"]').val().replace(/[^0-9]/g, '');
        var dadosConta = "";
        console.log(codBanco);

        //TRATAMENTO PARA AGENCIA, CONTA E DIGITOS
        if (codBanco == "246") {//DADOS AGENCIA E CONTA BB
            dadosConta = '<div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_246" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_246" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>';

        } else if (codBanco == "341") {//DADOS AGENCIA E CONTA ITAU
            dadosConta = ' <div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_341" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_341" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Digito da Conta:</label>'+
                '<input type="text" name="cont_conta_digito" id="ctd_341" class="form-control" maxlength="1" placeholder="Apenas números..." />'+
                '</div>';

        } else if (codBanco == "237") {//DADOS AGENCIA E CONTA BRADESCO
            dadosConta =  '<div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_237" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Digito da Agencia:</label>'+
                '<input type="text" name="cont_agencia_digito" id="agd_237" class="form-control"  maxlength="1" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_237" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Digito da Conta:</label>'+
                '<input type="text" name="cont_conta_digito" id="ctd_237" class="form-control"  maxlength="1" placeholder="Apenas números..." />'+
                '</div>';

        } else if (codBanco == "33") {//DADOS AGENCIA E CONTA SANTANDER
            dadosConta = '<div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_33" class="form-control" maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_33" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>';
        } else if (codBanco == "104") {//DADOS AGENCIA E CONTA CAIXA
            dadosConta =  '<div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_104" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_104" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Digito da Conta:</label>'+
                '<input type="text" name="cont_conta_digito" id="ctd_104" class="form-control"  maxlength="1" placeholder="Apenas números..." />'+
                '</div>';

        } else if (codBanco == "399") {//DADOS AGENCIA E CONTA HSBC
            dadosConta = '<div class="form-group col-md-3">'+
                '<label>Agencia:</label>'+
                '<input type="text" name="cont_agencia" id="ag_399" class="form-control" maxlength="8" placeholder="Apenas números..." />'+
                '</div>'+
                '<div class="form-group col-md-3">'+
                '<label>Conta:</label>'+
                '<input type="text" name="cont_conta" id="ct_399" class="form-control"  maxlength="8" placeholder="Apenas números..." />'+
                '</div>';
        }

        $("#dadosConta").html();
        $("#dadosConta").html(dadosConta);
    });

    $(document).change(function() {
        $("#ag_246").keypress(apenasNumeros);
        $("#ct_246").keypress(apenasNumeros);

        $("#ag_341").keypress(apenasNumeros);
        $("#ct_341").keypress(apenasNumeros);
        $("#ctd_341").keypress(apenasNumeros);

        $("#ag_237").keypress(apenasNumeros);
        $("#agd_237").keypress(apenasNumeros);
        $("#ct_237").keypress(apenasNumeros);
        $("#ctd_237").keypress(apenasNumeros);

        $("#ag_33").keypress(apenasNumeros);
        $("#ct_33").keypress(apenasNumeros);

        $("#ag_104").keypress(apenasNumeros);
        $("#ct_104").keypress(apenasNumeros);
        $("#ctd_104").keypress(apenasNumeros);

        $("#ag_399").keypress(apenasNumeros);
        $("#ct_399").keypress(apenasNumeros);
    });
