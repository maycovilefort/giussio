...
$Read->FullRead("SELECT lanc_id, lanc_desc, DATE_FORMAT(lanc_data_venc,'%d/%m/%Y') as lanc_data_venc, lanc_valor, lanc_tipo, DATE_FORMAT(lanc_data_pagto,'%d/%m/%Y') as lanc_data_pagto FROM lancamentos WHERE lanc_data_venc <= '{$DataInicial}' ORDER BY lanc_data_venc ASC");
$saldoAnterior = 0;
        ...
        <div class="box-body table-responsive no-padding">
            <table class="table table-hover table-striped">
                ...
                <tr>
                    ...
                    <th class="text-center" style="width: 12%">Ação</th>

                </tr>
                <?php
                $Read->FullRead("SELECT lanc_id, lanc_desc, lanc_data_venc, lanc_valor, lanc_tipo, DATE_FORMAT(lanc_data_pagto,'%d/%m/%Y') as lanc_data_pagto, lanc_id_imov, lanc_id_cond FROM lancamentos WHERE lanc_data_venc BETWEEN '{$DataInicial}' AND '{$DataFinal}' ORDER BY lanc_data_venc ASC");
                if ($Read->getResult()):
                    foreach ($Read->getResult() as $Dados):
                        extract($Dados);
                        ?>
                        <tr>
                            ...
                            <td class="text-center text-blue text-bold">
                                <?php
                                if ($lanc_tipo === TipoLancamento::RECEITA) {
                                    echo 'R$ ' . number_format($lanc_valor, 2, ',', '.');
                                    $saldoAnterior += $lanc_valor;
                                } else {
                                    echo '-';
                            ...
                            <td class="text-center">
                                ...
                                <button id="lancamentos" rel="<?= $lanc_id ?>" action="Update"
                                        class="btn btn-default btn-xs center j_baixar"><span class="fa fa-check"></span>
                                </button>
                                <button id="lancamentos" rel="<?= $lanc_id ?>" action="Delete"
                                        class="btn btn-default btn-xs center j_del"><span class="fa fa-close"></span>
                                </button>
                            </td>
                        </tr>
                        <?php
